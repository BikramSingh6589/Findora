 ...
 */
const processAIData = async (itemId, type, isWorkerJob = false) => {
    ...
    try {
        ...
        if (item.images && item.images.length > 0) {
            ...
            const ocrPromises = item.images.map(async (img) => {
                ...
                return (0, ocr_service_1.extractTextFromImage)(preprocessedImg, context);
            });
...
const triggerMatching = async (itemId, type) => {
    try {
        ...
        for (const target of targets) {
            ...
            const cleanTokens = (t) => t.toLowerCase()
                .replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, ' ')
                .split(/\s+/)
                .filter(Boolean)
                .filter(w => w.length >= 3)
...
const checkColorConflict = (strA, strB) => {
...
const checkBrandConflict = (brandA, brandB) => {
...
const checkModelConflict = (textA, textB) => {
...
const checkMaterialConflict = (textA, textB) => {
...
const checkSizeConflict = (textA, textB) => {
