 ...
 */
export const preprocessImageBuffer = async (imageBuffer: Buffer): Promise<Buffer> => {
  try {
 ...
 */
export const preprocessImage = async (imagePathOrUrl: string): Promise<any> => {
  const raw = await getImageBuffer(imagePathOrUrl);
  if (!raw) return imagePathOrUrl;
  return preprocessImageBuffer(raw);
};
 ...
 */
export const isLikelyTextImage = async (buffer: Buffer, itemContext?: string): Promise<boolean> => {
...
export const extractTextFromImage = async (
  ...
  try {
    ...
    const textCheck = await isLikelyTextImage(rawBuffer, itemContext);
    if (!textCheck) {
      return { extractedText: '' };
    }

    const preprocessedBuffer = await preprocessImageBuffer(rawBuffer);
